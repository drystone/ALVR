...
// asynchronously
fn update_streaming_input(ctx: &mut StreamingInputContext) -> StrResult {
    ...
    let now = xr_runtime_now(&ctx.xr_instance).ok_or_else(enone!())?;
                    ...
                    ClientCoreEvent::StreamingStarted {
                        ...
                        streaming_input_thread = Some(thread::spawn(move || {
                            ...
                            while is_streaming.value() {
                                show_err(update_streaming_input(&mut context));
